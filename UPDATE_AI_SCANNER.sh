...
echo "==================================================="
echo "   UPGRADING AI INVOICE SCANNER (MATH + DATE FIX)"
echo "==================================================="

# 1. Write the IMPROVED invoice_scanner.py to the HOST disk
echo "[1/4] Writing NEW invoice_scanner.py with MATH FALLBACK, STRICT PROMPTS & VALIDATION..."

cat <<'EOF' > backend/app/services/invoice_scanner.py
import os
import io
import json
import base64
...
# 2. Force copy to container
echo "[2/4] Copying UPDATED invoice_scanner.py to container..."
docker cp backend/app/services/invoice_scanner.py retailstore_backend:/app/app/services/invoice_scanner.py

# 3. Verify Copy
echo "[3/4] Verifying update..."
docker exec retailstore_backend grep "VALIDATION" /app/app/services/invoice_scanner.py
if [ $? -eq 0 ]; then
    echo "✅ CONFIRMED: AI Scanner Code is inside container."
else
...
docker restart retailstore_backend

echo "==================================================="
echo "   AI SCANNER UPGRADED. READY TO TEST."
echo "==================================================="
