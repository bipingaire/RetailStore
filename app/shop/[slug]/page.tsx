...
export default function ShopHome() {
  ...
  useEffect(() => {
    async function loadData() {
      // 1. Fetch Active Promotions (The "Flash Saves")
      const { data: promoData, error: promoError } = await supabase
        ...
        .select(`
          ...
          store_inventory ( 
            price, 
            global_products ( name, image_url, upc_ean ) 
          )
        `)
        .eq('is_active', true)
        .gt('end_date', new Date().toISOString()); // Only future deals

      if (promoError) console.error("Promo Error:", promoError);

      // 2. Fetch Regular Products (Limit 20 for MVP)
      ...
      const normalizedPromos: Promotion[] = (promoData ?? []).map((promo) => {
        const inventory = Array.isArray(promo.store_inventory)
          ? promo.store_inventory[0]
          : promo.store_inventory;
        const product = Array.isArray(inventory?.global_products)
          ? inventory?.global_products[0]
        ...
        return {
          ...
          fulfillment_restriction: promo.fulfillment_restriction ?? 'all',
          store_inventory: {
      ...
      const normalizedProducts: Product[] = (prodData ?? []).map((prod) => {
        const product = Array.isArray(prod.global_products)
          ? prod.global_products[0]
          : prod.global_products;
