...
import { Search, Calendar, ChevronDown, ChevronUp, FileText, Receipt } from 'lucide-react';
...
export default function InvoiceHistoryPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        // Fetch invoice list from FastAPI
        ...
        if (data.invoices) {
          ...
          const mappedInvoices = data.invoices.map((inv: any) => ({
            id: inv.invoice_id,
            vendor_name: inv.supplier || 'Unknown',
            invoice_number: inv.invoice_number || 'N/A',
            invoice_date: inv.created_at ? new Date(inv.created_at).toLocaleDateString() : '',
            total_amount: inv.total_amount || 0,
            line_items_json: [], // Will be loaded on expand
  ...
  // Load line items when expanding
  const handleRowExpand = async (invoiceId: string) => {
    ...
    if (invoice && invoice.line_items_json && invoice.line_items_json.length > 0) {
      return;
    }
    ...
    try {
      ...
      if (data.line_items_json) {
        setInvoices(prev => prev.map(inv =>
          inv.id === invoiceId
            ? { ...inv, line_items_json: data.line_items_json }
            : inv
        ));
      }
    } catch (error) {
                ...
                return (
                  <>
                    <tr
                      ...
                      onClick={() => handleRowExpand(inv.id)}
                    >
                    ...
                    {/* EXPANDED DETAILS */}
                    {isExpanded && inv.line_items_json && inv.line_items_json.length > 0 && (
